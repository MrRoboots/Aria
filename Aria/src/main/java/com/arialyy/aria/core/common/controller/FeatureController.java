...
public abstract class FeatureController {
     ...
     */
    public static <T extends FeatureController> T newInstance(@ControllerType Class<T> clazz, AbsTaskWrapper wrapper) {
    ...
    boolean checkConfig() {
        ...
        if (!b && schedulers != null) {
            new Handler(Looper.getMainLooper(), schedulers).obtainMessage(ISchedulers.CHECK_FAIL, checkTaskType(), -1, null).sendToTarget();
        }
    ...
    private boolean checkPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            // 处理 Android 14 特定的权限
            if (AriaConfig.getInstance().getAPP().checkCallingOrSelfPermission(Manifest.permission.READ_MEDIA_IMAGES) != PackageManager.PERMISSION_GRANTED || AriaConfig.getInstance().getAPP().checkCallingOrSelfPermission(Manifest.permission.READ_MEDIA_VIDEO) != PackageManager.PERMISSION_GRANTED || AriaConfig.getInstance().getAPP().checkCallingOrSelfPermission(Manifest.permission.READ_MEDIA_AUDIO) != PackageManager.PERMISSION_GRANTED) {
                ALog.e(TAG, "启动失败，缺少权限：READ_MEDIA_*");
                return false;
            }
        } else {
            // 处理 Android 10 及以下版本的权限
