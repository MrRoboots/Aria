...
public class M3U8VodDLoadActivity extends BaseActivity<ActivityM3u8VodBinding> {
    ...
    protected void init(Bundle savedInstanceState) {
        ...
        getBinding().pl.setBtListener(new ProgressLayout.OnProgressLayoutBtListener() {
            ...
            public void resume(View v, AbsEntity entity) {
                Aria.download(this).load(mTaskId).m3u8VodOption(getM3U8Option()).resume();
            }
    ...
    public void chooseUrl() {
        ModifyUrlDialog dialog = new ModifyUrlDialog(this, getString(R.string.modify_url_dialog_title), mUrl);
        dialog.show(getSupportFragmentManager(), "ModifyUrlDialog");
    }

    public void chooseFilePath() {
        ModifyPathDialog dialog = new ModifyPathDialog(this, getString(R.string.modify_file_path), mFilePath);
    ...
    void taskFail(DownloadTask task, Exception e) {
        if (task != null && task.getKey().equals(mUrl)) {
            Toast.makeText(M3U8VodDLoadActivity.this, getString(R.string.download_fail), Toast.LENGTH_SHORT).show();
            Log.d(TAG, "fail");
            getBinding().pl.setInfo(task.getEntity());
        }
    }
    ...
    void taskComplete(DownloadTask task) {
        if (task.getKey().equals(mUrl)) {
            Toast.makeText(M3U8VodDLoadActivity.this, getString(R.string.download_success), Toast.LENGTH_SHORT).show();
    ...
    private void startD() {
        mTaskId = Aria.download(M3U8VodDLoadActivity.this).load(mUrl).setFilePath(mFilePath).ignoreFilePathOccupy().m3u8VodOption(getM3U8Option()).create();
    }
